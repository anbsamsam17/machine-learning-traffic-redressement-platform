...
# ---------------------------------------------------------------------------
# PL-specific barplot + folium map helpers (siblings of the TV helpers above)
# ---------------------------------------------------------------------------

def _make_barplot_html_PL(df: pd.DataFrame, title: str) -> str:
    """Grouped bar chart TMJOBCPL vs DPL (max 200 sensors), Plotly fragment."""
    import plotly.graph_objects as go
    import plotly.io as pio

    d = df.copy()
    d["TMJOBCPL"] = pd.to_numeric(d.get("TMJOBCPL"), errors="coerce")
    d["DPL"] = pd.to_numeric(d.get("DPL"), errors="coerce")
    d = d.dropna(subset=["TMJOBCPL", "DPL"])
    if d.empty:
        return f"<p>Aucune donnee ({title})</p>"
    ...
    hover_cols = [
        c for c in [
            "PTM_ID", "Identifiant", "STA", "Type", "Commune", "Route",
            "TMJOFCDTV", "TMJOFCDPL", "TMJAFCDTV", "TMJAFCDPL",
            "car_count", "car_average_speed_kmh", "car_average_distance_km",
            "truck_count", "truck_average_speed_kmh",
            "truck_min_average_distance_km",
            "TMJOBCPL", "DPL", "TP_redressement",
            "Erreur %", "Erreur absolue", "GEH",
            "DPLmin", "DPLmax", "Tolerance_IN_OUT",
        ] if c in d.columns
    ]
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=labels, y=d["TMJOBCPL"].tolist(),
        name=f"{_label('TMJOBCPL')} (validation)", marker_color="#1f77b4",
        customdata=customdata,
        hovertemplate=hover_template,
    ))
    fig.add_trace(go.Bar(
        x=labels, y=d["DPL"].tolist(),
        name="DPL (predit)", marker_color="#00b894",
        customdata=customdata,
        hovertemplate=hover_template,
    ))
    fig.update_layout(
...
def _make_folium_map_html_PL(stats_df: pd.DataFrame, model_name: str) -> str:
    """Folium map for PL: colours by Tolerance_IN_OUT, sized by TMJOBCPL."""
    import folium
    ...
    # PL reference flow: TMJOBCPL (FCD HERE) — single canonical name.
    s_raw = valid.get("TMJOBCPL")
    if s_raw is None:
        s = pd.Series([np.nan] * len(valid), index=valid.index)
    else:
        s = pd.to_numeric(s_raw, errors="coerce")
    ...
    layer = folium.FeatureGroup(name="Capteurs PL")
    layer.add_to(m)
    ...
    for _, row in valid.iterrows():
        ...
        folium.CircleMarker(
            location=(row["lat"], row["lon"]),
            radius=_radius(row.get("TMJOBCPL")),
            color=_color(row.get("Tolerance_IN_OUT")),
            fill=True,
            fill_opacity=0.85,
            weight=1.2,
    ...
    <div style="position:fixed;bottom:20px;left:20px;z-index:9999;background:white;
            padding:10px 14px;border:1px solid #ccc;border-radius:10px;
            box-shadow:0 2px 8px rgba(0,0,0,.18);font-size:13px;font-family:Manrope,sans-serif;">
      <div style="font-weight:700;margin-bottom:7px;">Tolerance PL &ndash; {_html.escape(model_name)}</div>
      <div><span style="display:inline-block;width:13px;height:13px;background:#2ecc71;border:1px solid #999;margin-right:6px;border-radius:50%;"></span>1 Inclus <b>({n1} &ndash; {pct(n1):.1f}%)</b></div>
      <div><span style="display:inline-block;width:13px;height:13px;background:#f39c12;border:1px solid #999;margin-right:6px;border-radius:50%;"></span>2 Hors &lt;15% borne <b>({n2} &ndash; {pct(n2):.1f}%)</b></div>
      <div><span style="display:inline-block;width:13px;height:13px;background:#e74c3c;border:1px solid #999;margin-right:6px;border-radius:50%;"></span>3 Hors &gt;15% borne <b>({n3} &ndash; {pct(n3):.1f}%)</b></div>
      <div style="margin-top:6px;font-size:11px;color:#666;">Total: {n_valid} | Rayon ~ TMJOBCPL</div>
    </div>
    """
    m.get_root().html.add_child(folium.Element(legend_html))
...
# ---------------------------------------------------------------------------
# PL-specific HTML report (sibling of _generate_html_report — TV untouched)
# ---------------------------------------------------------------------------

def _generate_html_report_PL(
    metrics: MetricsResult,
    model_name: str,
    training_config: dict[str, Any] | None,
    y_true: np.ndarray,
    ...
    """Self-contained HTML evaluation report for Poids Lourds (PL) models.

    Mirrors ``_generate_html_report`` (TV) one-to-one but operates on the PL
    column set: predicted ``DPL``, reference ``TMJOBCPL``, tolerance band
    ``DPLmin`` / ``DPLmax``, FCD source ``TMJOFCDPL``, target ``TxPenPL``.
    The TV function is kept verbatim — never call this one for a TV model.
    """
    from ..services.ml.types import PL_CONFIG

    # --- Build stats row from PL columns ---
    if df is not None and "DPL" in df.columns and "TMJOBCPL" in df.columns:
        flow_metrics = _compute_flow_metrics(df, type_config=PL_CONFIG)
        tol_counts = _compute_tolerance_counts(df)
        ...
        # Fallback from y_true / y_pred arrays (no df).
        nonzero = y_true != 0
        if nonzero.any():
            rel_errors = np.abs((y_true[nonzero] - y_pred[nonzero]) / y_true[nonzero]) * 100
            n_total_pct = len(rel_errors)
    ...
    # --- Card styling helpers (identical thresholds — relative-error scale
    # behaves the same for PL since we use err_rel %, not absolute volume). ---
    def _card_style(v, good_thresh, mid_thresh, higher_is_better=False):
        if v is None or (isinstance(v, float) and (math.isnan(v) or math.isinf(v))):
            return ""
        if higher_is_better:
    ...
    # --- Barplot (PL) ---
    if df is not None and "DPL" in df.columns and "TMJOBCPL" in df.columns:
        bar_html = _make_barplot_html_PL(df, title=f"{model_name} - validation")
    else:
        bar_html = "<p>Aucune donnee disponible.</p>"

    # --- Folium map (PL) ---
    if df is not None and "lat" in df.columns and "lon" in df.columns:
        map_html = _make_folium_map_html_PL(df, model_name)
    else:
        map_html = "<p style='color:#888;font-style:italic;'>Donnees non disponibles pour la carte.</p>"

    # --- Outlier table (Erreur % > 15%) — identical logic, the column name
    # "Erreur %" is the same generic column the upstream code builds for both
    # TV and PL. ---
    if df is not None and "Erreur %" in df.columns:
        out_df = df.copy()
        out_df["Erreur %"] = pd.to_numeric(out_df.get("Erreur %"), errors="coerce")
        ...
        if not out_df.empty:
            ...
            outlier_html = "<p style='color:#2ecc71;font-weight:600;'>Aucun capteur PL avec une erreur &gt; 15%.</p>"
            outlier_count = 0
    else:
        outlier_html = "<p style='color:#888;font-style:italic;'>Donnees non disponibles.</p>"
        outlier_count = 0

    # --- Comparison table (single PL model row) ---
    header_cells = [
        "Modele PL", "N", "Err.rel med (%)", "Err.abs med",
        "Err.rel p80 (%)", "Err.abs p80", "GEH<5 (%)", "GEH<=10 (%)",
        "Err<10% N", "Err<10% %", "Err<15% N", "Err<15% %", "Err<20% N", "Err<20% %",
        "Tol 1 Inclus", "Tol 2 Hors<15%", "Tol 3 Hors>15%", "Tol Total",
    ]
    ...
    # --- P1.2 Stratification table per TMJOBCPL bucket ---
    if metrics_by_tmja_bucket:
        _bucket_headers = [
            "Bucket TMJOBCPL", "N", "Tol. inclus (N)", "Tol. inclus (%)",
            "p80 err.rel (%)", "R&sup2;",
        ]
        _bucket_thead = (
            "<tr>" + "".join(f"<th>{h}</th>" for h in _bucket_headers) + "</tr>"
        ...
        bucket_table_html = (
            '  <h2>Metriques stratifiees par tranche de TMJOBCPL</h2>\n'
            '  <p class="hint">Memes metriques recalculees sur 4 buckets de '
            'volume de trafic PL observe. Permet de detecter un modele PL '
            'performant globalement mais defaillant sur les capteurs '
            'faible/forte densite PL. '
            'Une ligne sur fond orange indique moins de 10 echantillons '
            '(metriques peu fiables).</p>\n'
            '  <div class="panel">\n'
            '    <table id="tmjaBucketTable" class="display" style="width:100%">\n'
        ...
        bucket_table_html = (
            '  <h2>Metriques stratifiees par tranche de TMJOBCPL</h2>\n'
            '  <p class="hint" style="color:#888;font-style:italic;">'
            'Stratification PL indisponible : colonne TMJOBCPL absente des '
            'donnees de validation.</p>\n'
        )

    # --- P4.1 Calibration plot ---
    calibration_plot_inner = _make_calibration_plot_html(calibration_data)
    calibration_section_html = (
        '  <h2>Calibration PL : predit vs observe</h2>\n'
        '  <p class="hint">Chaque point est un capteur PL. La diagonale rouge '
        '<code>y = x</code> represente une prediction parfaite. Un nuage '
        'systematiquement en dessous (resp. au-dessus) indique un biais de '
        'sous-estimation (resp. sur-estimation) du trafic Poids Lourds.</p>\n'
        '  <div class="panel plot-wrap">\n'
        f'    {calibration_plot_inner}\n'
        '  </div>\n'
    )
    ...
    residuals_plot_inner = _make_residuals_by_fc_html(residuals_by_fc)
    residuals_section_html = (
        '  <h2>Residus PL par classe fonctionnelle</h2>\n'
        '  <p class="hint">Distribution des residus PL <code>pred &minus; obs</code> '
        'pour chaque classe fonctionnelle (FC). Une boite centree sur 0 indique '
        'un modele PL non biaise sur cette classe ; une boite decalee revele un '
        'biais systematique propre a la classe.</p>\n'
        '  <div class="panel plot-wrap">\n'
        f'    {residuals_plot_inner}\n'
        '  </div>\n'
    ...
    drift_inner = _make_drift_by_year_html(drift_by_year)
    drift_section_html = (
        '  <h2>Derive annuelle PL (metriques par annee)</h2>\n'
        '  <p class="hint">Memes metriques PL recalculees pour chaque annee '
        'presente dans le jeu de validation (au moins 10 echantillons). Une '
        'forte variation du R&sup2; ou du tol_in entre annees suggere une '
        'derive temporelle du modele PL.</p>\n'
        '  <div class="panel">\n'
        f'    {drift_inner}\n'
        '  </div>\n'
    )
...
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/>
  <title>Evaluation PL - {_html.escape(model_name)}</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link href="https://fonts.googleapis.com/css2?family=Manrope:wght@400;600;700;800&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="https://cdn.datatables.net/1.13.8/css/jquery.dataTables.min.css"/>
  <script src="https://cdn.plot.ly/plotly-latest.min.js"></script>
...
<body>
<div class="wrap">
  <h1>Evaluation Poids Lourds (PL) sur donnees de validation</h1>
  <div class="subtitle">Modele PL : <code>{_html.escape(model_name)}</code> | {row['n']} capteurs | Trafic Poids Lourds (DPL vs TMJOBCPL)</div>

  <div class="cards">
    <div class="card" style="background:linear-gradient(145deg,#eef6ff,#e6f0ff);border-color:#bfd5f3;">
      <div class="k">Modele PL evalue</div>
      <div class="v" style="font-size:18px;overflow-wrap:anywhere;">{_html.escape(model_name)}</div>
      <div class="best-tag">Tolerance max + erreurs minimisees</div>
    </div>
    <div class="card" style="{tol_style}">
      <div class="k">Capteurs PL tolerance inclus</div>
      <div class="v">{row['tol_in']}/{row['tol_total']} <small style="font-size:13px;color:#56637a;">({_fmt(tol_in_pct_val)}%)</small>{_ci_span('tol_in_pct', digits=2, suffix='%')}</div>
    </div>
    <div class="card" style="{err_style}">
      <div class="k">Err. rel. mediane PL</div>
      <div class="v">{_fmt(row.get('err_rel_med'))}%</div>
    </div>
    <div class="card">
      <div class="k">Err. rel. p80 PL</div>
      <div class="v">{_fmt(row.get('err_rel_p80'))}%{_ci_span('p80', digits=2, suffix='%')}</div>
    </div>
    <div class="card">
      <div class="k">R&sup2; PL</div>
      <div class="v">{_fmt(r2_val, digits=4)}{_ci_span('r2', digits=4)}</div>
    </div>
    <div class="card">
      <div class="k">GEH &lt; 5 (PL)</div>
      <div class="v">{_fmt(row.get('geh_lt5_pct'))}%</div>
    </div>
    <div class="card" style="{pct10_style}">
      <div class="k">Capteurs PL erreur &lt; 10%</div>
      <div class="v">{n10} <small style="font-size:14px;color:#56637a;">({_fmt(pct10)}%)</small></div>
    </div>
    <div class="card" style="{pct15_style}">
      <div class="k">Capteurs PL erreur &lt; 15%</div>
      <div class="v">{n15} <small style="font-size:14px;color:#56637a;">({_fmt(pct15)}%)</small></div>
    </div>
    <div class="card" style="{pct20_style}">
      <div class="k">Capteurs PL erreur &lt; 20%</div>
      <div class="v">{n20} <small style="font-size:14px;color:#56637a;">({_fmt(pct20)}%)</small></div>
    </div>
  </div>

  <h2>Tableau des metriques detaillees PL</h2>
  <p class="hint">Metriques calculees sur les donnees de validation Poids Lourds. Tolerance : 1 = inclus, 2 = hors &lt;15% borne, 3 = hors &gt;15% borne.</p>
  <div class="panel">
    <table id="valTable" class="display" style="width:100%">
      <thead>{thead}</thead>
      <tbody>{tbody_row}</tbody>
...
{residuals_section_html}
{drift_section_html}
  <h2>Barplot - TMJOBCPL vs DPL (validation)</h2>
  <div class="panel plot-wrap">
    {bar_html}
  </div>

  <h2>Capteurs PL avec ecart &gt; 15% ({outlier_count} capteur(s))</h2>
  <p class="hint">Liste triee par erreur decroissante. Fond rose = erreur &gt; 50%, fond orange clair = erreur &gt; 30%.</p>
  <div class="panel">
    {outlier_html}
  </div>

  <h2>Carte des capteurs PL (validation)</h2>
  <p class="hint">Cliquez sur un capteur pour voir toutes ses informations. Couleur = tolerance (vert = inclus, orange = hors &lt;15%, rouge = hors &gt;15%). Taille = TMJOBCPL.</p>
  <div class="panel" style="padding:6px;overflow:hidden;">
    {map_html}
  </div>
...
async def run_evaluation(
    ...
    # Generate HTML report — dispatch TV vs PL from training_config.output_cols.
    # When the first output column contains "pl" (case-insensitive, matches
    # TxPenPL / DPL targets), route to the dedicated PL report so DPL /
    # TMJOBCPL / DPLmin / DPLmax columns are read correctly. TV path is the
    # default and stays bit-identical to the legacy behaviour.
    _target_lower = ""
    if training_config and isinstance(training_config, dict):
        _out_cols = training_config.get("output_cols") or []
        if _out_cols:
    ...
    if "pl" in _target_lower:
        report_html = _generate_html_report_PL(
            metrics=metrics,
