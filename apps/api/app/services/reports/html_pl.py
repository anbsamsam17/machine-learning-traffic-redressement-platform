"""HTML report generator — PL (Poids Lourds) models (daily, v/j, DPL vs TMJOBCPL).

Sibling of :mod:`html_tv` for the PL model variant. Operates on:

* predicted column: ``DPL``
* reference column: ``TMJOBCPL``
* tolerance band: ``DPLmin`` / ``DPLmax``
* FCD source: ``TMJOFCDPL``
* training target: ``TxPenPL``

Never call this generator for a TV / HPM / HPS model — the dispatcher in
:mod:`apps.api.app.routers.evaluation` selects the right flavour based on
``training_config.model_kind`` / ``output_cols``.
"""
...
def generate_html_report_pl(
    ...
    Mirrors ``generate_html_report_tv`` one-to-one but operates on the PL
    ...
    """
    from ..ml.types import PL_CONFIG
