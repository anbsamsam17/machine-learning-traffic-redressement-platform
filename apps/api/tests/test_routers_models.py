...
class TestModelsList:
    ...
    @pytest.mark.asyncio
    async def test_session_id_unknown_returns_200_empty(self, authenticated_client):
    ...
    @pytest.mark.asyncio
    async def test_arbitrary_dir_outside_workspace_returns_403(self, authenticated_client, tmp_path):
...
class TestModelsUpload:
    @pytest.mark.asyncio
    async def test_non_zip_returns_400(self, authenticated_client):
    ...
    @pytest.mark.asyncio
    async def test_invalid_zip_returns_400(self, authenticated_client):
        r = await authenticated_client.post(
            "/api/models/upload",
    ...
    @pytest.mark.asyncio
    async def test_empty_zip_accepts_but_zero_models(self, authenticated_client):
        buf = io.BytesIO()
