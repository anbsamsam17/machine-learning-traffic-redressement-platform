"""Resume Worker A3 — Phase 5 architecture ablation.

Picks up where the previous A3 worker stopped. For each entry in `configs_A3.json`:
- Skip if `<run_name>/metrics.json`, `<run_name>/report.html`, `<run_name>/model/`
  exist AND `actual_model_name` looks valid (no duplicate-suffix bug).
- Otherwise re-train + evaluate + persist.

Parses report.html with a regex that tolerates the new CI95 <small> annotations.
"""
...
DEFAULT_DATASET = PROJECT_ROOT / "apps/web/public/test/lyon_allyears.geojson"
...
P80_RE = re.compile(r'Err\. rel\. p80</div>\s*<div class="v">\s*([\d.]+)\s*%')
...
def _is_run_complete(target: Path, expected_cfg: dict[str, Any]) -> bool:
    """Check whether a previous run produced consistent artefacts."""
    metrics = target / "metrics.json"
    report = target / "report.html"
    model_dir = target / "model"
    if not (metrics.exists() and report.exists() and model_dir.exists()):
        return False
    ...
    try:
        ...
        return False
    # Heuristic: the actual_model_name suffix must match the expected weight_decay.
    actual = doc.get("actual_model_name", "")
    wd = expected_cfg.get("weight_decay")
    if wd is not None:
        # When wd=1e-4, suffix should contain wd0.0001; when wd=1e-3, wd0.001.
        # Detect mismatch.
        wd_str = f"{wd:g}"
        # adamw suffix uses wd<value>
        if "adamw" in actual and f"wd{wd_str}" not in actual.replace(".", ""):
            # crude check: convert both to e-notation-stripped form
            wd_clean = wd_str.lstrip("0").lstrip(".")
            actual_low = actual.lower()
            if f"wd{wd_str}" not in actual_low and not _wd_match(actual_low, wd):
                return False
    return True


def _wd_match(actual: str, wd: float) -> bool:
    """Match weight_decay floats against the suffix produced by training_pipeline."""
    # Pipeline format examples: wd0.0001, wd0.001
    cand = f"wd{wd:g}"
    return cand in actual
...
def run_one(
    ...
    # Include weight_decay in the hash so different wd values produce different
    # server output dirs (avoids the cache-collision bug seen with wd4 vs wd3).
    hash_input = f"{run_name}|wd={cfg.get('weight_decay')}|cn={cfg.get('clipnorm')}|skip={cfg.get('use_skip_connection')}|nl={cfg.get('norm_layer')}|ds={cfg.get('dropout_schedule')}|act={cfg.get('activation')}"
    short = hashlib.md5(hash_input.encode()).hexdigest()[:8]
    ...
    while True:
        ...
        if time.time() - t0 > 3600:
            print(f"[{run_name}] TIMEOUT after 60min", flush=True)
    ...
    # Pick the newest one (most recent mtime) so we don't pick up a stale cached model.
    actual = max(sub_dirs, key=lambda p: p.stat().st_mtime).name
    ...
    m_inclus = TOL_RE.search(html)
    m_p80 = P80_RE.search(html)
    tol_in, tol_total = (
        (int(m_inclus.group(1)), int(m_inclus.group(2))) if m_inclus else (0, 0)
    )
    try:
    ...
    print(
        f"[{run_name}] DONE [{health}] tol={tol_in}/{tol_total} p80={p80_val} R2={metrics.get('r_squared', 0):.3f}",
        flush=True,
    )
    return summary
...
def _write_readme(target: Path, cfg: dict, summary: dict) -> None:
    ...
    lines.append("Sortie: `TxPen` — taux de penetration FCD/Boucle Comptage TV")
    lines.append("")
    ...
    lines.append("```")
    lines.append("")
    lines.append("## Metriques de validation (sur 3671 capteurs)")
    tt = max(summary["tol_total"], 1)
    lines.append(
        f"- Capteurs tolerance inclus: **{summary['tol_inclus']}/{summary['tol_total']}** "
    ...
    if m.get("median_relative_error") is not None:
        lines.append(f"- Erreur relative mediane: {m['median_relative_error']}%")
    if m.get("r_squared") is not None:
        lines.append(f"- R2: {m['r_squared']:.4f}")
    ...
    lines.append(f"- Duree d'entrainement: {summary['train_seconds']}s")
...
def main() -> int:
    ...
    p.add_argument("--agent-id", default="A3")
    p.add_argument("--configs", default=str(BATCH_DIR / "configs_A3.json"))
    p.add_argument("--dataset", default=str(DEFAULT_DATASET))
    p.add_argument(
        "--only",
        default=None,
        help="Comma-separated run_names to force-run (skips skip-detection).",
    )
    args = p.parse_args()
    ...
    forced = set((args.only or "").split(",")) if args.only else set()
    forced.discard("")
    ...
    for cfg in base_configs:
        ...
        if forced and run_name not in forced:
            continue
        if not forced and _is_run_complete(target, cfg):
    ...
    if not todo:
        ...
        return 0
    print(f"Will run {len(todo)} configs: {[c['run_name'] for c in todo]}", flush=True)
    ...
    for cfg in todo:
        try:
            ...
            print(f"[{cfg['run_name']}] EXC: {exc}", flush=True)
            runs.append({"run_name": cfg.get("run_name", "?"), "error": str(exc)})

    summary_path = BATCH_DIR / f"_summary_{args.agent_id}_resume.json"
    summary_path.write_text(json.dumps(runs, indent=2, default=str), encoding="utf-8")
    print(
        json.dumps(
            {"agent": args.agent_id, "n_runs": len(runs), "summary": str(summary_path)},
            indent=2,
        ),
        flush=True,
    )
